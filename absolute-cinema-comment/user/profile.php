...
checkRole('user'); //ใช่ user ไหม?

// ดึงข้อมูลผู้ใช้ปัจจุบัน เตรียมและรันคำสั่ง SQL เพื่อดึงข้อมูลผู้ใช้จาก users ตาม user_id ใน session โดยเก็บข้อมูลไว้ในตัวแปร $user
try {
    $userStmt = $pdo->prepare("SELECT * FROM users WHERE id = ?");
    $userStmt->execute([$_SESSION['user_id']]);
    $user = $userStmt->fetch();
    
    //ถ้าไม่พบผู้ใช้ ให้บังคับ logout
    if (!$user) {
...
} //เกิด error จาก PDO ให้แสดงข้อความและหยุด script
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        // ถ้ามีการเปลี่ยนรหัสผ่าน ถ้าใส่รหัสผ่านใหม่ จะ hash ก่อนบันทึกลงฐานข้อมูลพร้อมกับเบอร์โทร
        if (!empty($password)) {
        ...
        } //ถ้าไม่เปลี่ยนรหัสผ่าน จะอัปเดตเฉพาะเบอร์โทร
